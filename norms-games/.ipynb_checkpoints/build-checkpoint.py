...
def get_participants(identifier: str, threshold: int) -> List[str]:
  ...
  """
  q = prolog.query("getParticipants({},{},L)".format(identifier,threshold))
  q_list = list(q)
  q.close()
...
def get_roles(identifier: str) -> Dict[str, List[str]]:
...
def get_actions(identifier: str) -> Dict[str, List[str]]:
...
def get_control_rule_consequences(game: ExtensiveFormGame, identifier: str,
                                  expand_node: int, n: int) -> List[Any]:
...
def build_full_game(folder: str, identifier: str, max_rounds: int=10,
                    verbose: bool=False) -> ExtensiveFormGame:
  ...
  participants = get_participants(identifier)
  ...
  while expand_queue:
    ...
    actions = get_actions(identifier)
    ...
    for n in provisional_next_states:
      ...
      sln = get_control_rule_consequences(game, identifier, expand_node, n)
      ...
      for active_rule in sln:
        ...
        # if this is the first rule to be activated
        if not next_states:
          for i in range(len(consequences)):
            next_states.append(deepcopy(consequences[i]))
            next_states_prob.append(probabilities[i])
          continue
      ...
      # asserted when calling get_contr_rule_consequences
