...
payoff(A,X1),C is -2,{X2=X1+C}]). % cost of traveling between spots c=-1

rule(fishers,control,0,
if does(F1,A) and does(F2,A)
then [
    won_fight(F1) and payoff(F1,R11) and lost_fight(F2) and payoff(F2,R21) withProb P1,
    won_fight(F2) and payoff(F2,R22) and lost_fight(F1) and payoff(F1,R12) withProb P2
    ]
where [
    at(F1,S),at(F2,S),F1@<F2,fishing_spot(S),
    strength(F1,X1), strength(F2,X2),{P1=X1/(X1+X2)},{P2=X2/(X1+X2)},
    payoff(F1,R1),payoff(F2,R2),D is -6,productivity(S,H),
    {R11=R1+H},{R21=R2+D},{R12=R1+D},{R22=R2+H}
    ]
).

% payoff
rule(fishers,payoff,0,if role(A,fisher) then payoff(A,Y)
where [payoff(A,X),at(A,S),fishing_spot(S),findall(F,at(F,S),[A]),productivity(S,H),{Y=X+H}]).
