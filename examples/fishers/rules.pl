...
rule(fishers,control,1,if does(F1,go_to_spot(S)) and does(F2,go_to_spot(S))
...
rule(fishers,position,2,if participates(A) then role(A,announcer)
...
rule(fishers,control,2,if does(F,announce_spot(S))
then [announced(F,S) withProb 1] where []).

rule(fishers,control,2,if does(F1,go_to_spot(S)) and does(F2,go_to_spot(S))
then [won_race(F1) withProb 1]
where [F1\=F2,announced(F1,S)]).
