...
% boundary
rule(ipd-default,boundary,0,if agent(A) then participates(A) where []).

%position
rule(ipd-default,position,0,if participates(A) then role(A,prisoner) where []).

% choice
rule(ipd-default,choice,0,if role(P,prisoner) then can(P,cooperate) where []).
rule(ipd-default,choice,0,if role(P,prisoner) then can(P,defect) where []).

% control
% P1@<P2 avoids equivalent instantiations of some control rules
rule(ipd-default,control,0,
