...
% boundary
rule(ipd,boundary,0,if agent(Ag) then participates(Ag) where []).

% position
rule(ipd,position,0,if participates(Ag) then role(Ag,prisoner) where []).

% choice
rule(ipd,choice,0,if role(Ag,prisoner) then can(Ag,cooperate) where []).
rule(ipd,choice,0,if role(Ag,prisoner) then can(Ag,defect) where []).
...
where [P1@<P2,payoff(P1,X1),payoff(P2,X2),{Y11=X1+0,Y12=X2+9,Y21=X1+9,Y22=X2+0}]).
